...
{
    ReceiveQueue_ = osMessageQueueNew(5, sizeof(Frame), NULL);
}
 ...
 extern "C" void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
 {
     for (int i = 0; i < SerialDevice::Ins_Count; i++)
     {
         if (SerialDevice::obArr[i]->huart_ == huart)
         {
             ...
             if (SerialDevice::obArr[i]->rx_index == 3 && SerialDevice::obArr[i]->check_Byte[0] == 0x21 && SerialDevice::obArr[i]->check_Byte[1] == 0x40)
             {
                 SerialDevice::obArr[i]->length_flag = SerialDevice::obArr[i]->rxBuffer_[SerialDevice::obArr[i]->rx_index - 1]; // the length byte
                 SerialDevice::obArr[i]->total_length = SerialDevice::obArr[i]->length_flag + 6; // 头2 + ID1 +长度1 + 数据 + 尾2
             }
             ...
             if ((SerialDevice::obArr[i]->check_Byte[0] == 0x26 && SerialDevice::obArr[i]->check_Byte[1] == 0x2A))
             {
                 ...
                 if (SerialDevice::obArr[i]->rx_index > 0)
                 {
                     ...
                     rx_frame.length = SerialDevice::obArr[i]->rxBuffer_[3];
                     rx_frame.ID = SerialDevice::obArr[i]->rxBuffer_[2];
                     ...
                     status = osMessageQueuePut(SerialDevice::obArr[i]->ReceiveQueue_, &rx_frame, 0, 0);
                     if (status != osOK)
                     {
                         // try again
