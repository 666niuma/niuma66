 ...
 {   
    ...
    for(i = 0; i<4; i++)
    {
		if(i == 2)
		{
            ...
            {
                temp = fabsf(pid->motor.motor_set[i].pid_speed) / ChassisMotor.maxRpm_Left[i];
                pid->motor.motor_set[i].current_set = (uint16_t)(temp * 100);
            }
            else if(pid->motor.motor_set[i].pid_speed < 0)
            {
                temp = fabsf(pid->motor.motor_set[i].pid_speed) / ChassisMotor.maxRpm_Right[i];
                pid->motor.motor_set[i].current_set = (uint16_t)(temp * 100);
            }
...
void FSM_Init(FSM_APP *current_state_init)
{
	*current_state_init = start;
}
	...
	switch (*current_state)
	{
       ...
       Chassis_Task();
		...
		case final_to_start:
			...
			else if(path_ctrl == THE_PATH_ONE)
            {
			chassis_ctrl = Right_PATH;
            while(time  <= 270){ Chassis_Task();}
			time = 0;
...
// 	while (time <= 200)
// 	{
// 	ChassisMotor.chassisVy = -ChassisMotor.maxVy * Proportion;
// 	ChassisCalculate(0, ChassisMotor.chassisVy_calc, &ChassisMotor);
// 	Motor_Control(ChassisMotor.motor.motor_set[0].current_set,0,0,0);
// 	Data = Chassis_ReadData();
// 	//printf_DMA("%f\r\n", Data->motor.motor_fdb[0].speed_fdb);
...
// 	ChassisCalculate(0, ChassisMotor.chassisVy_calc, &ChassisMotor);
// 	Motor_Control(ChassisMotor.motor.motor_set[0].current_set,0,0,0);
// 	//printf_DMA("d:%f, %f\n", ChassisMotor.motor.motor_fdb[0].speed_fdb, ChassisMotor.motor.motor_set[0].speed_set);
// 	}
...
{
//	printf_DMA("%d\n", ChassisMotor.motor.motor_fdb[0].speed_fdb);
//	printf_DMA("%d\n", ChassisMotor.motor.motor_set[0].speed_set);
    ...
    {
        ChassisMotor.chassisVx = -ChassisMotor.maxVx * Proportion;
        ChassisMotor.chassisVy = 0;
    }
    ...
    if(ChassisMotor.chassisVx != 0)
        {
            ...
            //限幅
            if(ChassisMotor.chassisVx_calc >= (ChassisMotor.maxVx * Proportion))
            {
                ChassisMotor.chassisVx_calc = ChassisMotor.maxVx * Proportion;
            }
            else if(ChassisMotor.chassisVx_calc <= -(ChassisMotor.maxVx * Proportion))
            {
                ChassisMotor.chassisVx_calc = -ChassisMotor.maxVx *Proportion;
            }
        ...
        if(ChassisMotor.chassisVy != 0)
        {
            ...
            //限幅
            if(ChassisMotor.chassisVy_calc >= (ChassisMotor.maxVy * Proportion))
            {
                ChassisMotor.chassisVy_calc = ChassisMotor.maxVy * Proportion;
            }
            
            else if(ChassisMotor.chassisVy_calc <= -(ChassisMotor.maxVy * Proportion))
            {
                ChassisMotor.chassisVy_calc = -ChassisMotor.maxVy * Proportion;
            }
